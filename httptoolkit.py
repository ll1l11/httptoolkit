...
class HttpToolkit:
    ...
    def __init__(self):
        ...
        accept = ('text/html,application/xhtml+xml,application/xml;'
                  'q=0.9,*/*;q=0.8')
        user_agent = ('Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.13 '
    ...
    def get(self, url=None, data=None, cookie=None, referer=None, timeout=20):
        params = self._data2params(data)
        if '?' in url and params:
            url = '%s&%s' % (url, params)
        elif params:
            url = '%s?%s' % (url, params)
        return self._request('GET', url, '', cookie, referer, timeout)

    def post(self, url=None, data={}, cookie=None, referer=None, timeout=30):
        return self._request("POST",
                             url,
                             self._data2params(data),
                             cookie,
                             referer,
                             timeout)

    def _data2params(self, data={}):
        try:
    ...
    def _request(self, method='GET', url=None, params='', cookie=None,
                 referer=None, timeout=30):
        ...
        if(HttpToolkit.proxy):
            proxy_url = '{0}:{1}'.format(HttpToolkit.ip, HttpToolkit.port)
            self.conn = httplib.HTTPConnection(proxy_url, timeout=timeout)
        ...
        if set_cookies is not None:
            ...
            for morsel in morsels:
                key, value, path = \
                    morsel.key, morsel.value, morsel['path'].strip(', ')
        ...
        if response.status == 301 or response.status == 302:
            ...
            return self._request(method='GET', url=location, params=params,
                                 cookie=self.cookie, referer=url)

        content_type = response.getheader("Content-Type")
        charset = None
        if content_type is not None and content_type.find('charset') != -1:
